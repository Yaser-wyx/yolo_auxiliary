...
from utils.general import (set_seed, LOGGER, check_dataset, check_file, check_git_status, check_img_size,
                           check_requirements,
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    feature_lambda, auxiliary_start, with_auxiliary, random_layers, progressive = \
    ...
    if with_auxiliary:
        ...
        auxiliary_model = Model(auxiliary_model_cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), is_auxiliary=True).to(
            device)
        auxiliary_model.requires_grad_(False)  # without update auxiliary network

        model.set_auxiliary(auxiliary_model, random_layers, feature_lambda, auxiliary_start, progressive, epochs)
    # DP mode
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                pred = model(imgs, epoch=epoch)  # forward
        ...
        if RANK in [-1, 0]:
            ...
            print(f"ema auxiliary_model: {ema.ema.auxiliary_model}")
            ...
            if with_auxiliary:
                model.auxiliary_model = None  # 避免保存网络的时候，将辅助网络也保存
            # Save model
            ...
            model.auxiliary_model = auxiliary_model
            # Stop Single-GPU
    ...
    # end training -----------------------------------------------------------------------------------------------------
    if with_auxiliary:
        model.auxiliary_model = None  # 结束训练，去掉辅助网络
    if RANK in [-1, 0]:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--random-layers', type=int, default=2)  # currently only supported backbone
    parser.add_argument('--lambda_', type=float, default=0.1)
    parser.add_argument('--auxiliary-start', type=int, default=0)
    # parser.add_argument('--auxiliary-use-prob', type=float, default=0.8)
